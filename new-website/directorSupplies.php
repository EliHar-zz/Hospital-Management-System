...
<body>
    ...
    <?php
        ...
        if (isset($_POST['buy'])) {
            ...
            // insert into orders
            $query = "INSERT INTO orders VALUES($order_id, now(), $facility_id, $floor_num, $supply_id, $quantity)";
            ...
            $query = "SELECT * FROM floor_supplies
                      WHERE floors_facility_id=$facility_id
                      AND supplies_supply_id=$supply_id";
            ...
            if (mysqli_num_rows($result) > 0) {
                $query = "UPDATE floor_supplies
                          SET quantity=quantity+$quantity
                          WHERE floors_facility_id=$facility_id
                          AND floors_floor_number=$floor_num
                          AND supplies_supply_id=$supply_id";
                ...
                $query = "INSERT INTO floor_supplies VALUES($facility_id, $floor_num, $supply_id, $quantity)";
                $result = mysqli_query($con, $query) or die("Unable to insert new purchase");
            }
        }

        $query = "SELECT supply_name, quantity
                  FROM floor_supplies, supplies
                  WHERE supply_id=supplies_supply_id
                  AND floors_facility_id=1
                  AND floors_floor_number=1";
    ...
    <div style="text-align:center">
        ...
        <form class="form" method="POST" action="<?php $_SERVER['PHP_SELF'] . "?fac=$facility" ?>">
            ...
            <div class="row">
                ...
                <div class="col right">
                    <h4>Add to Floor Supplies</h4><br/>
                    <?php
                        echo $vendor;
                    ?>
                </div><br/>
