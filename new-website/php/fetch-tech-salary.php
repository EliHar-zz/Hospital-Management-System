...
$start_date_string = $start;//$_SESSION['start_date'];
$start_date_date = date('Y/m/d', strtotime($start_date_string));
$years_of_service = floor((date('Y/m/d') - $start_date_date));

$sql = "SELECT base_hourly_rate, raise_amount, years_to_raise, hours_per_week FROM technicians WHERE employee_types_id = '".$_SESSION['searched']['employee_type_id']."'";
